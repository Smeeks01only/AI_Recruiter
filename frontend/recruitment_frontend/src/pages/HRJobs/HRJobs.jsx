              ...
              <div className="job-card-creator">
                <div className="job-card-creator-profile">
                  {job.created_by_id?.profile_image ? (
                    <img
                      src={job.created_by_id.profile_image}
                      alt="Profile"
                      className="creator-avatar"
                    />
                  ) : (
                    <div className="creator-avatar-placeholder" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#f1f5f9', color: '#64748b' }}>
                      <PersonOutlineIcon style={{ fontSize: '1.2rem' }} />
                    </div>
                  )}

                  <div className="job-card-creator-info">
                    <p>
                      {job.created_by_id?.first_name || "HR"}{" "}
                      {job.created_by_id?.last_name || "Admin"}
                    </p>
                    <span>{job.created_by_id?.email || "admin@company.com"}</span>
                  </div>
                </div>
